...
#Version = Tuple[int, Any, Optional[int], bool]  
# (write_ts, value, expire_ts, is_deleted)

class MiniDB:
    """
    Super-basic staged solution:
      - Stage 1: set / get
      - Stage 2: TTL (lazy)
      - Stage 3: get_at (point-in-time)
      - Stage 4: delete via tombstone

    All time handling is explicit via a 'now'/'t' integer you pass in tests,
    which keeps behavior deterministic for unit tests.
    """

    def __init__(self):
        # key -> list of versions in increasing write_ts
        self.store = {}

    # ---- Stage 1 ----
    def set(self, key: str, value: Any, now: int, ttl: Optional[int] = None) -> None:
        """
        Write value at logical time 'now'. If ttl is provided, it expires at now + ttl.
        Expiry is exclusive: visible while t < expire_ts.
        """
    ...
    def get(self, key: str, now: int) -> Optional[Any]:
        """
        Return latest visible value at time 'now' (Stage 1 + Stage 2).
        """
        return self.get_at(key, now)

    # ---- Stage 3 ----
    def get_at(self, key: str, t: int) -> Optional[Any]:
        """
        Return the value visible at point-in-time 't'.
        Walk versions newest->oldest until you find a visible one.
        """
        versions = self.store.get(key, [])
        for write_ts, value, expire_ts, is_deleted in reversed(versions):
            if write_ts <= t:
                if is_deleted:
                    return None
                if expire_ts is None or t < expire_ts:  # exclusive expiry
                    return value
                # otherwise expired at t; keep scanning older versions
        return None

    # ---- Stage 4 ----
    def delete(self, key: str, now: int) -> None:
        """
        Append a tombstone. Older PIT reads still see older values.
        """
